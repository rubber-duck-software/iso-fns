...
describe('DayOfWeek: to-ordinal-1', ({ it }) => {
  it('should return the correct ordinal from day-of-week assuming 0-indexing', () => {
    ...
    expect(toOrdinal(Monday)).toBe(1)
    expect(toOrdinal(Tuesday)).toBe(2)
    expect(toOrdinal(Wednesday)).toBe(3)
    expect(toOrdinal(Thursday)).toBe(4)
    expect(toOrdinal(Friday)).toBe(5)
