...
describe('Instant', () => {
  describe('instantFns.from() works', () => {
    ...
    it('instantFns.from(string-convertible) converts to string', () => {
      ...
      assert.equal(`${instantFns.from(obj)}`, '2020-02-12T10:42:00Z')
    })
    ...
    it('instantFns.from(instant) is not the same object', () => {
      const inst = instantFns.from('2020-02-12T11:42+01:00[Europe/Amsterdam]')
      assert.notEqual(instantFns.from(inst), inst)
    })
    it('instantFns.from(ISO string leap second) is constrained', () => {
      assert.equal(`${instantFns.from('2016-12-31T23:59:60Z')}`, '2016-12-31T23:59:59Z')
    })
    it('variant time separators', () => {
      assert.equal(`${instantFns.from('1976-11-18t15:23Z')}`, '1976-11-18T15:23:00Z')
      assert.equal(`${instantFns.from('1976-11-18 15:23Z')}`, '1976-11-18T15:23:00Z')
    })
    it('variant UTC designator', () => {
      assert.equal(`${instantFns.from('1976-11-18T15:23z')}`, '1976-11-18T15:23:00Z')
    })
    it('any number of decimal places', () => {
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.1Z')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.12Z')}`, '1976-11-18T15:23:30.12Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.123Z')}`, '1976-11-18T15:23:30.123Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.1234Z')}`, '1976-11-18T15:23:30.1234Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.12345Z')}`, '1976-11-18T15:23:30.12345Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.123456Z')}`, '1976-11-18T15:23:30.123456Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.1234567Z')}`, '1976-11-18T15:23:30.1234567Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.12345678Z')}`, '1976-11-18T15:23:30.12345678Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.123Z')}`, '1976-11-18T15:23:30.123Z')
    })
    ...
    it('variant minus sign', () => {
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.12\u221202:00')}`, '1976-11-18T17:23:30.12Z')
      assert.equal(`${instantFns.from('\u2212009999-11-18T15:23:30.12Z')}`, '-009999-11-18T15:23:30.12Z')
    })
    it('mixture of basic and extended format', () => {
      assert.equal(`${instantFns.from('19761118T15:23:30.1+00:00')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('1976-11-18T152330.1+00:00')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.1+0000')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('1976-11-18T152330.1+0000')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('19761118T15:23:30.1+0000')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('19761118T152330.1+00:00')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('+0019761118T15:23:30.1+00:00')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('+001976-11-18T152330.1+00:00')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('+001976-11-18T15:23:30.1+0000')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('+001976-11-18T152330.1+0000')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('+0019761118T15:23:30.1+0000')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('+0019761118T152330.1+00:00')}`, '1976-11-18T15:23:30.1Z')
      assert.equal(`${instantFns.from('+0019761118T152330.1+0000')}`, '1976-11-18T15:23:30.1Z')
    })
    it('optional parts', () => {
      assert.equal(`${instantFns.from('1976-11-18T15:23:30+00')}`, '1976-11-18T15:23:30Z')
      assert.equal(`${instantFns.from('1976-11-18T15Z')}`, '1976-11-18T15:00:00Z')
    })
    it('ignores any specified calendar', () =>
      assert.equal(`${instantFns.from('1976-11-18T15:23:30.123Z[u-ca=discord]')}`, '1976-11-18T15:23:30.123Z'))
  ...
  describe('Instant.add works', () => {
    ...
    describe('cross epoch in ms', () => {
      const one = instantFns.subtract(inst, { hours: 240, milliseconds: 800 })
      const two = instantFns.add(inst, { hours: 240, milliseconds: 800 })
      const three = instantFns.subtract(two, { hours: 480, milliseconds: 1600 })
      const four = instantFns.add(one, { hours: 480, milliseconds: 1600 })
      it(`(${inst}).subtract({ hours: 240, milliseconds: 800 }) = ${one}`, () =>
        assert.equal(`${one}`, '1969-12-15T12:23:45.678900434Z'))
      it(`(${inst}).add({ hours: 240, milliseconds: 800 }) = ${two}`, () =>
        assert.equal(`${two}`, '1970-01-04T12:23:45.678902034Z'))
      it(`(${two}).subtract({ hours: 480, milliseconds: 1600 }) = ${one}`, () => assert(instantFns.equals(three, one)))
      it(`(${one}).add({ hours: 480, milliseconds: 1600 }) = ${two}`, () => assert(instantFns.equals(four, two)))
    })
    it('inst.add(durationObj)', () => {
      const later = instantFns.add(inst, durationFns.from('PT240H0.000000800S'))
      assert.equal(`${later}`, '1970-01-04T12:23:45.678902034Z')
    })
    ...
    it('incorrectly-spelled properties are ignored', () => {
      //@ts-expect-error
      assert.equal(`${instantFns.add(inst, { hour: 1, minutes: 1 })}`, '1969-12-25T12:24:45.678901234Z')
    })
  })
  describe('Instant.subtract works', () => {
    const inst = instantFns.from('1969-12-25T12:23:45.678901234Z')
    it('instantFns.subtract(inst, durationObj)', () => {
      const earlier = instantFns.subtract(inst, durationFns.from('PT240H0.000000800S'))
      assert.equal(`${earlier}`, '1969-12-15T12:23:45.678900434Z')
    })
    ...
    it('incorrectly-spelled properties are ignored', () => {
      //@ts-expect-error
      assert.equal(`${instantFns.subtract(inst, { hour: 1, minutes: 1 })}`, '1969-12-25T12:22:45.678901234Z')
    })
  })
  ...
  describe("Comparison operators don't work", () => {
    const i1 = instantFns.from('1963-02-13T09:36:29.123Z')
    const i1again = instantFns.from('1963-02-13T09:36:29.123Z')
    const i2 = instantFns.from('1976-11-18T15:23:30.123Z')
    it('=== is object equality', () => assert.equal(i1, i1))
    it('!== is object equality', () => assert.notEqual(i1, i1again))
    it('<', () => assert.throws(() => i1 < i2))
    it('>', () => assert.throws(() => i1 > i2))
    it('<=', () => assert.throws(() => i1 <= i2))
    it('>=', () => assert.throws(() => i1 >= i2))
  })
  ...
  describe('Instant.until() works', () => {
    ...
    it(`(${earlier}).until(${later}) == (${later}).since(${earlier})`, () =>
      assert.equal(`${instantFns.since(later, earlier)}`, `${diff}`))
    it(`(${earlier}).add(${diff}) == (${later})`, () => assert(instantFns.equals(instantFns.add(earlier, diff), later)))
    it(`(${later}).subtract(${diff}) == (${earlier})`, () =>
      assert(instantFns.equals(instantFns.subtract(later, diff), earlier)))
    const feb20 = instantFns.from('2020-02-01T00:00Z')
  ...
  describe('Instant.round works', () => {
    ...
    it('rounding down is towards the Big Bang, not towards the epoch', () => {
      ...
      assert.equal(`${instantFns.round(inst2, { smallestUnit, roundingMode: 'ceil' })}`, '1969-12-15T12:00:01Z')
      assert.equal(`${instantFns.round(inst2, { smallestUnit, roundingMode: 'floor' })}`, '1969-12-15T12:00:00Z')
      assert.equal(`${instantFns.round(inst2, { smallestUnit, roundingMode: 'trunc' })}`, '1969-12-15T12:00:00Z')
      assert.equal(`${instantFns.round(inst2, { smallestUnit, roundingMode: 'halfExpand' })}`, '1969-12-15T12:00:01Z')
    })
    it('rounds to an increment of hours', () => {
      assert.equal(`${instantFns.round(inst, { smallestUnit: 'hour', roundingIncrement: 4 })}`, '1976-11-18T16:00:00Z')
    })
    it('rounds to an increment of minutes', () => {
      assert.equal(`${instantFns.round(inst, { smallestUnit: 'minute', roundingIncrement: 15 })}`, '1976-11-18T14:30:00Z')
    })
    it('rounds to an increment of seconds', () => {
      assert.equal(`${instantFns.round(inst, { smallestUnit: 'second', roundingIncrement: 30 })}`, '1976-11-18T14:23:30Z')
    })
    it('rounds to an increment of milliseconds', () => {
      assert.equal(
    ...
    //   assert(instantFns.round(inst, { smallestUnit: 'second', roundingIncrement: 864 }) instanceof Instant)
    // })
  ...
  describe('Min/max range', () => {
    ...
    //   assert.throws(() => new Instant(-limit - 1n), RangeError)
    //   assert.throws(() => new Instant(limit + 1n), RangeError)
    //   assert.equal(`${new Instant(-limit)}`, '-271821-04-20T00:00:00Z')
    //   assert.equal(`${new Instant(limit)}`, '+275760-09-13T00:00:00Z')
    // })
    it('constructing from ms', () => {
      const limit = 86400e11
      assert.throws(() => instantFns.fromEpochMilliseconds(-limit - 1), RangeError)
      assert.throws(() => instantFns.fromEpochMilliseconds(limit + 1), RangeError)
      assert.equal(`${instantFns.fromEpochMilliseconds(-limit)}`, '-271821-04-20T00:00:00Z')
      assert.equal(`${instantFns.fromEpochMilliseconds(limit)}`, '+275760-09-13T00:00:00Z')
    })
    it('constructing from ISO string', () => {
      assert.throws(() => instantFns.from('-271821-04-19T23:59:59.999999999Z'), RangeError)
      assert.throws(() => instantFns.from('+275760-09-13T00:00:00.000000001Z'), RangeError)
      assert.equal(`${instantFns.from('-271821-04-20T00:00Z')}`, '-271821-04-20T00:00:00Z')
      assert.equal(`${instantFns.from('+275760-09-13T00:00Z')}`, '+275760-09-13T00:00:00Z')
    })
    it('converting from DateTime', () => {
      const min = zonedDateTimeFns.from('-271821-04-19T00:00:00.000000001')
      const max = zonedDateTimeFns.from('+275760-09-13T23:59:59.999999999')
  ...
  describe('Instant.toZonedDateTime() works', () => {
    ...
    it('throws without parameter', () => {
      //@ts-expect-error
      assert.throws(() => instantFns.toZonedDateTime(inst), TypeError)
    })
    it('throws with a string parameter', () => {
      assert.throws(() => instantFns.toZonedDateTime(inst, 'Asia/Singapore'), TypeError)
    })
  })
})
